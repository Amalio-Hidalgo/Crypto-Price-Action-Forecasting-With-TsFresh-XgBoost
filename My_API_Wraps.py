...
def CoinGecko_HSPD_Dask(timeframe, top_coins, periods, api_key, timezone='CET'):
    ...
    for coin in coins:
        ...
        # dd_response['datetime']= dd_response['datetime'].apply(lambda x: x.tz_convert(tz='CET'), meta = ('datetime', 'datetime64[ns, CET]'))
        # dd_response['datetime']= dd_response['datetime'].map(lambda x: x.tz_convert('CET'), meta = ('datetime', 'datetime64[ns, CET]'))
        # dd_response['datetime']= dd_response['datetime'].map(lambda x: x.compute().tz_convert('CET'), meta = ('datetime', 'datetime64[ns, CET]'))
    ...
    output = output.repartition(npartitions= 20)
    return output

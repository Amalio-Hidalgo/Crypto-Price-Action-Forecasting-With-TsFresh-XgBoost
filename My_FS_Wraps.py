...
def SF_Pandas_Simple(features, timeframe=1, forecast_periods=1, p_value=0.05, target='prices ethereum', LR=True, Vol=False):
    ...
    FC= features.dropna(subset='y_future').dropna(subset='LR(%)').dropna(subset='1PeriodVol(%)')
    ...
    if target != '1PeriodVol(%)':selected_features.append('1PeriodVol(%)')
...
def SF_Dask_Simple(features, timeframe=1, forecast_periods=1, p_value=0.05, target='prices ethereum',LR=True, Vol=False):
    target_var = features[target]
    features['LR(%)']= np.log(target_var)- np.log(target_var.shift(1))
    features['LR(%)']=features['LR(%)']*100
    features['1PeriodVol(%)']= abs(features['LR(%)'])

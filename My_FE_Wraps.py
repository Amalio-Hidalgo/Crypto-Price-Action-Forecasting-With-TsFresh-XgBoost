...
def EF_Dask(df, ParameterComplexity=1):
    ...
    features= features.reset_index(drop=True).astype({'variable': 'string', 'value': 'float', 'id': 'string'})
    ...
    features=features.repartition(npartitions=12)
    return features
...
def EF_Pandas_MultiprocessingDistributor(df, ParameterComplexity=1):
    ...
    rolled = roll_time_series(df, column_id='variable', column_sort='datetime', n_jobs=14)
